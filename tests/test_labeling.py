...
from docrouter.labeling.prioritize import build_queue
...
def test_wilson_interval_widens_on_small_samples(tmp_path):
    store = LabelStore(tmp_path / "labels.jsonl")
    for i in range(4):
        store.append(LabelRecord(doc_id=f"d{i}", label="gosi", model_label="gosi", lane="random"))
    narrow_lo, _narrow_hi = store.stats().random.wilson_interval()
    for i in range(4, 60):
        store.append(LabelRecord(doc_id=f"d{i}", label="gosi", model_label="gosi", lane="random"))
    wide_lo, _wide_hi = store.stats().random.wilson_interval()
...
def test_low_confidence_outranks_high_confidence():
    docs = _docs(12)
    preds = [
        _pred(d.doc_id, "moj_courts", 0.95 if n % 2 == 0 else 0.30) for n, d in enumerate(docs)
    ]
...
def test_per_class_cap_limits_one_class_dominating():
    ...
    items = build_queue(docs, predictions=preds, size=20, random_ratio=0.0, per_class_cap=4, seed=1)
    assert len(items) <= 4
...
def _session(tmp_path, lane="random", blind=True):
    ...
    items = build_queue(
        [doc], predictions=[pred], size=1, random_ratio=1.0 if lane == "random" else 0.0
    )
    items[0].lane = lane
    store = LabelStore(tmp_path / "labels.jsonl")
    return (
        ReviewSession(items, store, taxonomy.load(), "tester", blind_random=blind),
        items[0],
        store,
    )
...
def test_gold_export_feeds_the_evaluator(tmp_path):
    ...
    labeled = [Document(doc_id=d.doc_id, text=d.text, true_label=gold[d.doc_id]) for d in docs]
